        ...
        <form>
            ...
            <select name="agent_id" id="agent_id">
                ...
                @foreach($agents as $items)
                    <option value="{{ $items->id }}"
                            @if(isset($c_agent) && $items->id == $c_agent->id) selected="selected" @endif>{{ $items->nickname }}</option>
                @endforeach
            </select>
            ...
            <select name="issue" id="issue">
                ...
                @foreach($issues as $items)
                    <option value="{{ $items->id }}"
                            @if(isset($c_issue) && $items->id == $c_issue->id) selected="selected" @endif>{{ $items->id }}</option>
                @endforeach
            </select>
    ...
    <div role="main" class="ui-content">
        <div style="margin:-1em -.5em;">
            <div class="conditions">
                ...
                @endif
                @if(isset($c_account))
                    <p><span class="label">用户：</span><span class="condition">{{ $c_account->nickname }}</span></p>
                @endif
                @if(isset($c_agent))
                    <p><span class="label">代理人：</span><span class="condition">{{ $c_agent->nickname }}</span></p>
                @endif
                @if(isset($c_game))
            ...
            <ul data-role="listview" data-inset="true">
                @foreach($orders as $order)
                    <li>
                        <h3>{{ $order->game->name.'-'.$order->issue.'期' }}</h3>
                        <div class="items">
