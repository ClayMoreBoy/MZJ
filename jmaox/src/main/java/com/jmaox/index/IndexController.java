...
import com.jmaox.utils.AgentUtil;
import com.jmaox.utils.DateUtil;
import com.jmaox.utils.EmailSender;
...
public class IndexController extends BaseController {
    ...
    /**
     * 后台管理登录 默认账号admin 默认密码123123 对应表 admin_user
     */
...
//            AdminUser adminUser = AdminUser.me.login(username, HashKit.md5(password));
            ...
            if (adminUser == null) {
                ...
                if (!StrUtil.isBlank(before_url) && !before_url.contains("adminlogin")) {
                    redirect(before_url);
                }
    ...
    public void forgetpwd() {
        ...
        if (method.equalsIgnoreCase(Constants.RequestMethod.GET)) {
            if (!AgentUtil.getAgent(getRequest()).equals(AgentUtil.WEB)) {
                render("mobile/user/forgetpwd.html");
            } else {
